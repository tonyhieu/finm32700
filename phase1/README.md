...
4. Memory alignment is the practice of ensuring that addresses are divisible by specific values. In C++, we can use `posix_memalign()` to align memory. Memory alignment is most relevant for cache lines (e.g. if addresses are divisible by 64 bytes, then one line contains 64 bytes of information), as larger divisions lead to a lower chance of splitting contiguous data. In our benchmarks, aligning the memory did not show a significant improvement in performance. Some reasons why this can occur are that modern compilers optimize away alignment issues and that modern CPUs have a better tolerance for misaligned data.
...
6. Profiling confirmed our belief that column-major operations are much less efficient than row-major ones and that the transposed implementation of matrix multiplication performs somewhat better than the naive one. Because we did not use many function calls, the profiler did not reveal many inefficiencies in our implementation. Instead, we researched different optimization techniques to implement instead.
